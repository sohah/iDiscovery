 ...
 *
 * <p>An Option is not created independantly, but is create through an instance of {@link Options}.
 *
 * <p>
 *
 * @see Options
...
public class Option implements Cloneable, Serializable {
   ...
   *     </code>.
   */
  public Option(char opt, char longOpt, int args, String description)
  ...
  /**
   * Returns the id of this Option. This is only set when the Option shortOpt is a single character.
   ...
   *
   * <p>It is this String which can be used with {@link CommandLine#hasOption(String opt)} and
  ...
  /**
   * Processes the value. If this Option has a value separator the value will have to be parsed into
   * individual tokens. When n-1 tokens have been processed and there are more value separators in
   * the value, parsing is ceased and the remaining characters are added as a single token.
   *
  ...
  /**
   * Returns the value/first value of this Option or the <code>defaultValue</code> if there is no
   * value.
   *
   * @param defaultValue The value to be returned if ther is no value.
   * @return the value/first value of this Option or the <code>defaultValue</code> if there are no
   *     values.
   */
  ...
  /**
   * A rather odd clone method - due to incorrect code in 1.0 it is public and in 1.1 rather than
   * throwing a CloneNotSupportedException it throws a RuntimeException so as to maintain backwards
   * compat at the API level.
   *
   * <p>After calling this method, it is very likely you will want to call clearValues().
   *
  ...
  /**
   * Clear the Option values. After a parse is complete, these are left with data in them and they
   * need clearing if another parse is done.
   *
   * <p>See: <a href="https://issues.apache.org/jira/browse/CLI-71">CLI-71</a>
   */
  void clearValues() {
    values.clear();
  }
  ...
  /**
   * This method is not intended to be used. It was a piece of internal API that was made public in
   * 1.0. It currently throws an UnsupportedOperationException.
   *
   * @deprecated
