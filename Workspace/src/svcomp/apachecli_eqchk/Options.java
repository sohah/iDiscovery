 ...
 *
 * <p>Options represents a collection of {@link Option} objects, which describe the possible options
 * for a command-line.
 *
 * <p>
 *
 * <p>It may flexibly parse long and short options, with or without values. Additionally, it may
 * parse only a portion of a commandline, allowing for flexible multi-stage parsing.
 *
 * <p>
 *
 * @see CommandLine
...
public class Options implements Serializable {
  ...
  /**
   * Retrieve the {@link Option} matching the long or short name specified. The leading hyphens in
   * the name are ignored (up to 2).
   *
  ...
  public Option getOption(char opt) {
    ...
    Option result = null;
    if (temp_Boolean) {
      result = shortOptsGet(opt);
//      return result;
    } else
    result = longOptsGet(opt);
    return result;
