...
public abstract class Parser implements CommandLineParser {
   ...
   * @since 1.1
   */
  public CommandLine parse(Options _options, char[][] arguments, Properties properties)
   ...
   * @param _options the <code>Options</code>
   * @param arguments the <code>arguments</code>
   * @param stopAtNonOption specifies whether to stop interpreting the arguments when a non option
   *     has been encountered and to add them to the CommandLines args list.
   * @return the <code>CommandLine</code>
   * @throws ParseException if an error occurs when parsing the arguments.
   */
  public CommandLine parse(Options _options, char[][] arguments, boolean stopAtNonOption)
      throws ParseException {
   ...
   * @param arguments the command line arguments
   * @param properties command line option name-value pairs
   * @param stopAtNonOption stop parsing the arguments when the first non option is encountered.
   * @return the list of atomic option and value tokens
   * @throws ParseException if there are any problems encountered while parsing the command line
   *     tokens.
   * @since 1.1
  ...
  public CommandLine parse(
      Options _options, char[][] arguments, Properties properties, boolean stopAtNonOption)
      throws ParseException {
    //        char c = arguments[0][0];
    //        if ( c == '*') stopAtNonOption = true;
    //
    ...
    while (_next) {
      ...
      if (arg == '-') {
        ...
        if (_next) {
          ...
          arg = t[0];
          //                    if (arg == '-') stopAtNonOption = true; // only gets veritested
          // twice, other times the condition is concrete, how ?
          if (arg == '-') {
            eatTheRest = true;
          } else if (arg == ' ') {
            if (stopAtNonOption) {
  ...
  /**
   * Process the argument values for the specified Option <code>opt</code> using the values
  ...
  /**
   * Process the Option specified by <code>arg</code> using the values retrieved from the specfied
