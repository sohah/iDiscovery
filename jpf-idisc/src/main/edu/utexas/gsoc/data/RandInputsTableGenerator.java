...
public class RandInputsTableGenerator {
	...
	public static void main(String[] args) throws IOException {
		...
		genData(paper + "tables/" + "randinputdata.tex");
	...
	private static void genTable(String f) throws IOException {
		for (String tech : techs) {
			...
			if (tech.contains("rand05-"))
				writer.write("\\caption{\\label{tab:"
						+ tech
						+ "}Experimental results for \\idisc{} with 5\\% random inputs}\n");
			else if (tech.contains("rand10-"))
				writer.write("\\caption{\\label{tab:"
						+ tech
						+ "}Experimental results for \\idisc{} with  10\\% random inputs}\n");
			else if (tech.contains("rand15-"))
				writer.write("\\caption{\\label{tab:"
						+ tech
						+ "}Experimental results for \\idisc{} with  15\\% random inputs}\n");
			else if (tech.contains("rand20-"))
				writer.write("\\caption{\\label{tab:"
						+ tech
						+ "}Experimental results for \\idisc{} with  20\\% random inputs}\n");
			else
				writer.write("\\caption{\\label{tab:"
						+ tech
						+ "}Experimental results for \\idisc{} with  100\\% random inputs}\n");
				...
				for (int i = 0; i < vs.length; i++) {
					...
					for (int iter = 0; iter < iterNum[i]; iter++) {
						...
						writer.write("&" + prefix + "TestNum &" + prefix
								...
								+ prefix + "TestMem &" + prefix + "TestGenTime");
						if (iter == (iterNum[i] - 1)) {
							/*
			...
			for (int s = 0; s < curSubs.length; s++) {
				...
				for (int i = 0; i < vs.length; i++) {
					...
					for (int iter = 0; iter < iterNum[i]; iter++) {
						...
						SymbcResParser.parseSymbcOut(symbcFile, tech, ver,
								iter, writer, curSub);
