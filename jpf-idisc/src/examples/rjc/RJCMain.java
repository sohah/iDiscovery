...
public class RJCMain {
	...
	public void DoSim() {
		...
		try {
			...
			BufferedWriter out = new BufferedWriter(new FileWriter(
					"JavaOutput.csv")); // Write the second output to a file
			...
			for (int i = 0; i < cmdInputs.size() && i < measInputs.size(); i++) {
				...
				for (int j = 0; j < 3; j++) {
					...
					input2[j] = currMeas.get(j + 1); // Throw away the time
														// stamp
				}
				this.controller.Main0(input1, input2, output1, output2);

				System.out.println("Output 1 (yaw jets)   : " + output1[0]);
				System.out.println("Output 2 (pitch jets) : " + output2[0]
						+ ", " + output2[1]);
				out.write("" + currCmd.get(0) + "," + output2[0] + ","
						+ output2[1] + "\n");
			}
	...
	// Symbolic pathfinder cannot handle arrays, so use this method if you want
	// to run the process symbolically
	public void DoSimSymbolic() {
		double[] out1 = new double[1];
		double[] out2 = new double[2];

		for (int i = 0; i < 1; i++) { // orig: 10
			out1 = new double[1];
			out2 = new double[2];
	...
	public void DoSimSymbolic(double p1, double p2, double p3, double p4, double p5, double p6) {
		double[] out1 = new double[1];
		double[] out2 = new double[2];
		...
		for (int i = 0; i < 1; i++) { // orig: 10
			out1 = new double[1];
			out2 = new double[2];
			this.controller.MainSymbolic(p1, p2, p3, p4, p5, p6, out1, out2);
		}
	}
